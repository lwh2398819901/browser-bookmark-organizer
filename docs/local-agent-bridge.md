...
`bookmarkctl.py` 在单次命令期间监听随机端口，使用指定 Chromium 浏览器启动一个默认最小化的小型应用窗口。该本地页面向固定扩展 ID 发送请求，扩展校验安装器生成的随机令牌后执行并返回 JSON。收到结果后，本地服务立即关闭，窗口尝试自动关闭。部分桌面环境可能短暂显示任务栏图标，但不要求用户切换窗口或点击页面。
