...
当前 2.0.7 只实现本地使用方案：本机 Agent 加载技能，通过短时 localhost 桥接调用本地扩展。它不需要长期开放端口，不连接远端收藏夹服务，也不建设插件内置的云端 Agent。

2.0.6 的工具栏在当前页已收藏时显示“取消收藏”。它按规范化网址直接删除这一页的全部匹配（跟踪参数不同也算同一条），不进入 `回收站`，也不能撤销。助手执行的删除计划仍默认移入 `回收站`，只有显式 `--purge` 才永久删除。2.0.5 在原有移动闭环上增加软删除、回收站清空、空目录清理、目录重命名和目录移动。删除和目录变更都先给出预览；执行前自动归档，并可通过操作记录撤销。计划令牌有效期为 30 分钟。全库扫描仍覆盖全部根目录，变更仅支持收藏夹栏内条目。离线 HTML 重组尚未提供正式工具。
...
- 快速收藏与防重复：从工具栏把当前网页加入 `临时收藏`；全库已有相同规范化网址时显示原位置，按钮变为“取消收藏”。取消收藏会直接删除这些匹配，不进入回收站。
...
- 删除与目录维护：整理时的删除候选先移入 `待删除`，由用户手工删除；删完后剩余条目放回合适位置，空文件夹再删掉。用户点名要助手直接删除时，书签默认移入 `回收站`。空目录可清理；目录可重命名和移动。永久删除需要单独确认，执行前都会先归档。
