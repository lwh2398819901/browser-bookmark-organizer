...
2.0.6 的工具栏在当前页已收藏时显示“取消收藏”，只移除这个网址的精确匹配。2.0.5 在原有移动闭环上增加软删除、回收站清空、空目录清理、目录重命名和目录移动。删除和目录变更都先给出预览；执行前自动归档，并可通过操作记录撤销。书签默认移入 `回收站`，只有显式 `--purge` 才永久删除。计划令牌有效期为 30 分钟。全库扫描仍覆盖全部根目录，变更仅支持收藏夹栏内条目。离线 HTML 重组尚未提供正式工具。
...
- 快速收藏与防重复：从工具栏把当前网页加入 `临时收藏`；全库已有相同网址时显示原位置，按钮变为“取消收藏”，不再创建副本。
