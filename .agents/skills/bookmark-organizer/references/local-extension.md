...
- 书签软删除写入收藏夹栏下的 `回收站`，并保留原 ID。`validate-plan --purge` 或 `purge-recycle` 才会永久删除；永久删除后的撤销会重建条目并分配新的运行时 ID。空目录可用 `prune-folders --empty` 删除，撤销时重建空目录。目录可以重命名和移动，书签标题与 URL 仍不能改。`临时收藏` 和 `回收站` 不会被自动清理。离线 HTML 重组尚未提供正式工具，不能承诺自动生成整理后的导入文件。
