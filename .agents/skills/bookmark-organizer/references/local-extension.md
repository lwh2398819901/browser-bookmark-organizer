...
- `scan`、`status`、`folders`、`archives`、`downloads`、`operations` 是只读操作；`backup` 只创建文件。`apply-plan`、`undo`，以及带 `--confirmed` 的 `prune-folders`、`rename-folder`、`move-folder`、`purge-recycle` 会改动收藏夹，必须在用户确认预览后调用。不带 `--confirmed` 时这些目录命令只返回预览。
...
- 工具栏小窗口用于把当前网页加入 `临时收藏`、在已收藏时取消该页、备份和打开整理中心。取消收藏直接删除规范化网址的全部匹配，不进入 `回收站`。不同 URL 片段代表不同章节，不得作为重复阻止或一并取消。
...
- 备份列表中的每一份都可以单独“删除备份”。用户点击该按钮即明确指定了目标文件，插件会直接删除该备份且不再二次确认。整理中心也可以一次清空早于指定天数（默认 7 天）或落在指定日期内的本扩展备份。这些操作只删除备份文件，不影响收藏夹，也不得被当作清理收藏夹的方式。
...
- 每次插件整理在本地保存原目录与位置，最近 20 次显示在操作记录中。撤销前再次备份，按记录恢复移动、软删除、重命名和目录移动；永久删除会重建条目。不会删除整理过程中创建的空目录。
- 操作记录的“清空记录”只删除插件本地保存的这些记录，不动收藏夹、也不删除任何备份文件；用户点击该明确按钮后直接执行并显示页面内结果，不再弹出确认框。
- 整理时的删除候选由 Agent 移入收藏夹栏下的 `待删除`，用户手工删除后再收尾。助手直接删除时，书签软删除写入收藏夹栏下的 `回收站`，并保留原 ID。`validate-plan --purge` 或 `purge-recycle` 才会永久删除；永久删除后的撤销会重建条目并分配新的运行时 ID。空目录可用 `prune-folders --empty` 删除；`--path` 把范围限在该目录内，重复的 `--exclude` 跳过要保留的空目录。撤销时重建空目录。目录可以重命名和移动，书签标题与 URL 仍不能改。`临时收藏` 和 `回收站` 不会被自动清理。离线 HTML 重组尚未提供正式工具，不能承诺自动生成整理后的导入文件。
