...
- **优先使用文件输出**：需要让 Agent 读取 `scan`、`status` 或其他 JSON 结果时，可加 `--output <临时目录>\result.json`。`--output` 是全局参数，必须写在子命令之前（如 `--pretty --output <文件> scan`）；写在子命令之后会被 argparse 拒绝。脚本会直接写入无 BOM UTF-8 文件，避免 PowerShell 管道转码，也避免宿主工具不回传 stdout。写入后再用读取工具打开该文件；输出文件只能放在工作区或系统临时目录。
