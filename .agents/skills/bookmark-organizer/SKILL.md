...
description: 备份、审计、生成画像、去重并按内容语义整理浏览器收藏夹；适用于单独备份、首次全量整理或定期处理“临时收藏”。
---
...
- 用户只要求备份、归档或导出收藏夹时，进入“仅备份”模式，不附带整理、去重、链接检查或移动。优先使用配套扩展的“仅备份当前收藏夹”，生成完整、可重新导入的 HTML；成功后报告文件路径，并明确说明没有修改收藏夹。扩展不可用时可使用浏览器原生“导出收藏夹/书签”功能；不得通过构造空移动计划冒充备份。
