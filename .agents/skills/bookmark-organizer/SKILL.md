...
- 用户只要求备份、归档或导出收藏夹时，进入“仅备份”模式，不附带整理、去重、链接检查或移动。优先使用配套扩展的“一键备份收藏夹”，生成完整、可重新导入的 HTML；成功后报告文件路径，并明确说明没有修改收藏夹。扩展不可用时可使用浏览器原生“导出收藏夹/书签”功能；不得通过构造空移动计划冒充备份。
...
- 用户要求撤销插件刚执行的整理时，优先使用整理中心的操作记录；撤销前再次备份，并只恢复记录中的原父目录和位置。用户要求从 HTML 归档恢复时，说明浏览器导入通常会与当前收藏夹合并，不得自动清空当前收藏夹。
