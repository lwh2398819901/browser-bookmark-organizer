...
若用户只说“整理临时收藏”但没有提供扩展实时任务，优先使用当前 Agent 已具备的桌面或浏览器自动化能力，在用户可见的本机浏览器中打开配套扩展的整理中心，完成“扫描临时收藏”和“复制整理任务给 Agent”这两个只读步骤。当前 Agent 无法控制该浏览器时，再请用户完成这两个点击并粘贴任务内容。拿不到任务时不得改从离线文件获取 ID；用户只能提供标题或 URL 时，可以给出不含执行 ID 的分类建议，但不能声称它可直接执行。自动化获取任务不代表获得移动或删除授权。
